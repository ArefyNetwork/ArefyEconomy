package com.arefyeconomy.api;

import com.arefyeconomy.economy.EconomyManager;
import com.arefyeconomy.util.RateLimiter;

import javax.annotation.Nonnull;
import java.util.UUID;

/**
 * Public API for ArefyEconomy economy plugin.
 * 
 ...
 * - Throws ArefyEconomyRateLimitException if rate limit exceeded
 * 
 * Thread Safety:
 ...
 *     try {
 *         boolean success = ArefyEconomyAPI.deposit(playerUuid, 100.0, "Quest reward");
 *     } catch (ArefyEconomyRateLimitException e) {
 ...
 * </pre>
 */
public final class ArefyEconomyAPI {
    
    private static EconomyManager economyManager;
    private static RateLimiter rateLimiter;
    
    private ArefyEconomyAPI() {}
    
    /**
     * Initialize the API (called by ArefyEconomy plugin on startup).
     */
    ...
    public static String getCurrencySymbol() {
        return com.arefyeconomy.Main.CONFIG.get().getCurrencySymbol();
    }
    ...
    public static String getHudPrefix() {
        return com.arefyeconomy.Main.CONFIG.get().getHudPrefix();
    }
    ...
    public static String format(double amount) {
        return com.arefyeconomy.Main.CONFIG.get().format(amount);
    }
    ...
    public static String getLanguage() {
        return com.arefyeconomy.Main.CONFIG.get().getLanguage();
    }
    ...
    public static boolean isUsePlayerLanguage() {
        return com.arefyeconomy.Main.CONFIG.get().isUsePlayerLanguage();
    }
    ...
    public static double getMaxBalance() {
        return com.arefyeconomy.Main.CONFIG.get().getMaxBalance();
    }
     ...
     */
    public static java.util.List<com.arefyeconomy.economy.PlayerBalance> getTopBalances(int limit) {
        validateAvailable();
    ...
    public static double getTotalCirculating() {
        validateAvailable();
        return economyManager.getAllBalances().values().stream()
            .mapToDouble(com.arefyeconomy.economy.PlayerBalance::getBalance)
            .sum();
    }
    ...
    public static void resetBalance(@Nonnull UUID playerUuid, @Nonnull String reason) {
        validateAvailable();
        checkRateLimit(playerUuid);
        double startingBalance = com.arefyeconomy.Main.CONFIG.get().getStartingBalance();
     ...
     */
    public static java.util.List<com.arefyeconomy.economy.TransactionEntry> getTransactionHistory(
     ...
     * 
     * @return true if ArefyEconomyCoins addon is loaded and registered
     */
     ...
     * Called by ArefyEconomyCoins addon on startup.
     * 
    ...
    public static void registerPhysicalCoinsProvider(PhysicalCoinsProvider provider) {
        ...
        java.util.logging.Logger.getLogger("ArefyEconomy")
            .info("[ArefyEconomy] Physical Coins provider registered.");
    }
    
    /**
     * Unregister the physical coins provider.
     * Called by ArefyEconomyCoins addon on shutdown.
     */
    public static void unregisterPhysicalCoinsProvider() {
        if (coinsProvider != null) {
            java.util.logging.Logger.getLogger("ArefyEconomy")
                .info("[ArefyEconomy] Physical Coins provider unregistered.");
            coinsProvider = null;
        }
    }
    ...
    private static void validateAvailable() {
        if (!isAvailable()) {
            throw new IllegalStateException("ArefyEconomy is not initialized. " +
                "Ensure ArefyEconomy plugin is loaded before calling the API.");
        }
    }
    
    private static void checkRateLimit(UUID playerUuid) {
        if (!rateLimiter.tryAcquire(playerUuid)) {
            throw new ArefyEconomyRateLimitException(
