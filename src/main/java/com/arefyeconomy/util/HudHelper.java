...
public final class HudHelper {
    ...
    private static final String HUD_ID = "arefyeconomy";
    ...
    public static void init() {
        try {
            ...
            if (multipleHudInstance != null) {
                ...
                com.arefyeconomy.Main.getInstance().getLogger()
                    .at(Level.INFO).log("ArefyEconomy: MultipleHUD detected, using compatible mode!");
            }
        } catch (ClassNotFoundException e) {
            // MultipleHUD not installed - this is fine
            com.arefyeconomy.Main.getInstance().getLogger()
                .at(Level.INFO).log("ArefyEconomy: MultipleHUD not detected, using vanilla HUD");
        } catch (Exception e) {
            // Something went wrong with reflection - fall back to vanilla
            com.arefyeconomy.Main.getInstance().getLogger()
                .at(Level.WARNING).log("ArefyEconomy: Failed to initialize MultipleHUD compat: " + e.getMessage());
        }
    }
    ...
    public static void setCustomHud(Player player, PlayerRef playerRef, CustomUIHud hud) {
        if (multipleHudAvailable && multipleHudInstance != null && setCustomHudMethod != null) {
            try {
                ...
                com.arefyeconomy.Main.getInstance().getLogger()
                    .at(Level.WARNING).log("ArefyEconomy: MultipleHUD call failed, using vanilla: " + e.getMessage());
            }
        }
    ...
    /**
     * Remove the ArefyEconomy HUD for a player.
