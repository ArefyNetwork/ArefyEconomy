package com.arefyeconomy.hud;

import com.arefyeconomy.lib.simplehud.SimpleHud;
import com.arefyeconomy.lib.simplehud.HudScheduler;
...
public class BalanceHud extends SimpleHud {
    ...
    public BalanceHud(PlayerRef playerRef) {
        super(playerRef, "Pages/ArefyEconomy_BalanceHud.ui");
        this.ownerRef = playerRef;
        
        double currentBalance = com.arefyeconomy.Main.getInstance().getEconomyManager().getBalance(playerRef.getUuid());
    ...
    protected void build(com.hypixel.hytale.server.core.ui.builder.UICommandBuilder builder) {
        ...
        if (!com.arefyeconomy.Main.CONFIG.get().isEnableHudDisplay()) {
            return;
        }
        super.build(builder);
    }
    ...
    public void updateBalance(double newBalance) {
        ...
        if (!com.arefyeconomy.Main.CONFIG.get().isEnableHudAnimation()) {
    ...
    private void updateDisplayTrailing(double balance) {
        ...
        String symbol = com.arefyeconomy.Main.CONFIG.get().getCurrencySymbol();
        ...
        String hudPrefix = Messages.get("hud.prefix");
    ...
    private void updateDisplayFinal(double balance) {
        ...
        String formatted = com.arefyeconomy.Main.CONFIG.get().formatShort(balance);
        String symbol = com.arefyeconomy.Main.CONFIG.get().getCurrencySymbol();
        // Use per-player translation if available, otherwise config value
        String hudPrefix = Messages.get("hud.prefix");
    ...
    private void notifyDisabledOnce() {
        ...
        warnedDisabled = true;
        ownerRef.sendMessage(Message.raw(Messages.get("hud.disabled")));
    }
}
