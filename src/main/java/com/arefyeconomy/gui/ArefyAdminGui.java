...
import com.arefyeconomy.economy.TransactionEntry;
import com.arefyeconomy.economy.TransactionLogger;
import com.arefyeconomy.locale.Messages;
import com.arefyeconomy.systems.BalanceHudSystem;
...
/**
 * ArefyAdminGui - Complete admin panel for economy management.
 * 
 ...
 * - Force save button
 */
public class ArefyAdminGui extends InteractiveCustomUIPage<ArefyAdminGui.AdminGuiData> {
    ...
    public ArefyAdminGui(@NonNullDecl PlayerRef playerRef) {
    ...
    public void build(@NonNullDecl Ref<EntityStore> ref, @NonNullDecl UICommandBuilder cmd, 
        ...
        cmd.append("Pages/ArefyEconomy_AdminPage.ui");
    ...
    public void handleDataEvent(@NonNullDecl Ref<EntityStore> ref, @NonNullDecl Store<EntityStore> store, 
        ...
        if (data.action != null) {
            Main.getInstance().getLogger().at(java.util.logging.Level.INFO).log("ArefyAdminGui action received: %s", data.action);
            switch (data.action) {
                case "Close" -> {
                    this.close();
                    return;
                ...
                case "ForceSave" -> {
                    ...
                    playerRef.sendMessage(Message.raw(Messages.get("admin.save")).color(Color.GREEN));
                    return;
                }
                case "Give", "Take", "Set" -> {
                    if (selectedPlayerUuid != null) {
                        ...
                        if (amount >= 0) {  // -1 means invalid; 0 is valid for Set
                            ...
                            playerRef.sendMessage(Message.raw(Messages.get("admin.invalid_amount", "max",
                                Main.CONFIG.get().format(Main.CONFIG.get().getMaxBalance()))).color(Color.RED));
                        }
                    } else {
                        playerRef.sendMessage(Message.raw(Messages.get("admin.select_player")).color(Color.RED));
                    }
                    refreshUI(ref, store);
                    return;
                }
                ...
                case "ReloadConfig" -> {
                    Main.CONFIG.load();
                    Messages.reload();
                    playerRef.sendMessage(Message.raw(Messages.get("admin.reload")).color(Color.GREEN));
                    refreshUI(ref, store);
                    return;
                }
                case "SaveConfig" -> {
                    ...
                    playerRef.sendMessage(Message.raw(Messages.get("gui.config.saved")).color(Color.GREEN));
                    refreshUI(ref, store);
                    return;
                }
                case "ToggleHud" -> {
                    ...
                    String status = newValue ? Messages.get("gui.config.yes") : Messages.get("gui.config.no");
                    playerRef.sendMessage(Message.raw("HUD: " + status).color(Color.GREEN));
                    refreshUI(ref, store);
                    return;
                }
                case "ToggleLang" -> {
                    ...
                    playerRef.sendMessage(Message.raw(Messages.get("gui.config.language") + ": " + next).color(Color.GREEN));
                    refreshUI(ref, store);
                    return;
                }
                case "TogglePerPlayer" -> {
                    ...
                    String status = newValue ? Messages.get("gui.config.yes") : Messages.get("gui.config.no");
                    playerRef.sendMessage(Message.raw(Messages.get("gui.config.per_player") + ": " + status).color(Color.GREEN));
                    refreshUI(ref, store);
                    return;
                }
                case "ResetDefaults" -> {
                    ...
                    playerRef.sendMessage(Message.raw(Messages.get("gui.config.reset_done")).color(Color.YELLOW));
                    refreshUI(ref, store);
                    return;
                }
    ...
    private void buildDashboard(@NonNullDecl UICommandBuilder cmd) {
        ...
        if (recentActivity.isEmpty()) {
            cmd.appendInline("#ActivityLog",
                "Label { Text: \"" + Messages.get("gui.dashboard.no_activity") + "\"; Style: (FontSize: 11, TextColor: #888888); }");
        } else {
    ...
    private void buildPlayersTab(@NonNullDecl UICommandBuilder cmd, @NonNullDecl UIEventBuilder events) {
        ...
        // Update page info
        cmd.set("#PageInfo.Text", Messages.get("gui.players.page", "current", String.valueOf(currentPage + 1), "total", String.valueOf(totalPages)));
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            cmd.append("#PlayerList", "Pages/ArefyEconomy_AdminPlayerEntry.ui");
        ...
        if (selectedPlayerUuid != null && selectedPlayerName != null) {
            cmd.set("#SelectedLabel.Text", Messages.get("gui.players.selected") + ":");
            cmd.set("#SelectedName.Text", selectedPlayerName);

            // Update Reset button text based on confirmation state
            if (selectedPlayerUuid.equals(confirmingResetUuid)) {
                cmd.set("#ResetButton.Text", Messages.get("gui.players.reset_confirm"));
            } else {
                cmd.set("#ResetButton.Text", Messages.get("gui.players.reset"));
            }
        } else {
            cmd.set("#SelectedLabel.Text", Messages.get("gui.players.select_player"));
        ...
        if (displayIndex == 0) {
            cmd.appendInline("#PlayerList", "Group { LayoutMode: Center; Anchor: (Height: 60); " +
                "Label { Text: \"" + Messages.get("gui.players.no_players") + "\"; Style: (FontSize: 14, TextColor: #888888, HorizontalAlignment: Center); } }");
        }
    }
    ...
    private void buildLogTab(@NonNullDecl UICommandBuilder cmd) {
        ...
        if (h2Storage == null) {
            cmd.appendInline("#LogList", "Label { Text: \"" + Messages.get("gui.log.requires_h2") + "\"; Style: (FontSize: 14, TextColor: #888888); Padding: (Top: 20); }");
            return;
        }

        // Show loading state
        cmd.set("#LogCountInfo.Text", Messages.get("gui.log.loading"));
        ...
        h2Storage.countTransactionsAsync(filter).thenCombineAsync(
            ...
            (totalCount, entries) -> {
                ...
                if (totalCount == 0) {
                    asyncCmd.set("#LogCountInfo.Text", Messages.get("gui.log.no_transactions"));
                } else {
                    asyncCmd.set("#LogCountInfo.Text",
                        Messages.get("gui.log.showing", "start", String.valueOf(startIdx), "end", String.valueOf(endIdx),
                            "total", String.valueOf(totalCount), "page", String.valueOf(logPage + 1), "pages", String.valueOf(totalPages)));
                }

                if (entries.isEmpty()) {
                    if (logFilter.isEmpty()) {
                        asyncCmd.appendInline("#LogList",
                            "Label { Text: \"" + Messages.get("gui.log.no_recorded") + "\"; Style: (FontSize: 12, TextColor: #888888); Anchor: (Top: 20); }");
                    } else {
                        asyncCmd.appendInline("#LogList",
                            "Label { Text: \"" + Messages.get("gui.log.no_matches", "filter", logFilter) + "\"; Style: (FontSize: 12, TextColor: #888888); Anchor: (Top: 20); }");
                    }
                } else {
    ...
    private void buildConfigTab(@NonNullDecl UICommandBuilder cmd) {
        ...
        // Display fields
        String yesText = Messages.get("gui.config.yes");
        String noText = Messages.get("gui.config.no");
    ...
    private void translateUI(@NonNullDecl UICommandBuilder cmd) {
        // Header
        cmd.set("#Title.Text", Messages.get("gui.admin.title"));
        cmd.set("#ForceSaveButton.Text", Messages.get("gui.admin.force_save"));

        // Tab buttons
        cmd.set("#TabDashboard.Text", Messages.get("gui.admin.tab.dashboard"));
        cmd.set("#TabPlayers.Text", Messages.get("gui.admin.tab.players"));
        cmd.set("#TabTop.Text", Messages.get("gui.admin.tab.top"));
        cmd.set("#TabLog.Text", Messages.get("gui.admin.tab.log"));
        cmd.set("#TabConfig.Text", Messages.get("gui.admin.tab.config"));

        // Dashboard labels (only elements with IDs in the UI file)
        cmd.set("#LblTotalCirculating.Text", Messages.get("gui.dashboard.total_circulating"));
        cmd.set("#LblPlayersWithBalance.Text", Messages.get("gui.dashboard.players_with_balance"));
        cmd.set("#LblAverageBalance.Text", Messages.get("gui.dashboard.avg_balance"));
        cmd.set("#LblCurrentConfig.Text", Messages.get("gui.dashboard.current_config"));
        cmd.set("#LblRecentActivity.Text", Messages.get("gui.dashboard.recent_activity"));

        // Players tab
        cmd.set("#LblSearch.Text", Messages.get("gui.players.search_label"));
        cmd.set("#LblAmount.Text", Messages.get("gui.players.amount"));
        cmd.set("#GiveButton.Text", Messages.get("gui.players.give"));
        cmd.set("#TakeButton.Text", Messages.get("gui.players.take"));
        cmd.set("#SetButton.Text", Messages.get("gui.players.set"));
        cmd.set("#ResetButton.Text", Messages.get("gui.players.reset"));

        // Config tab section headers (only elements with IDs)
        cmd.set("#LblCurrency.Text", Messages.get("gui.config.currency"));
        cmd.set("#LblLimits.Text", Messages.get("gui.config.limits"));
        cmd.set("#LblDisplay.Text", Messages.get("gui.config.display"));
        cmd.set("#LblLanguage.Text", Messages.get("gui.config.language"));

        // Config tab buttons
        cmd.set("#SaveConfigButton.Text", Messages.get("gui.config.save"));
        cmd.set("#ResetDefaultsButton.Text", Messages.get("gui.config.reset_defaults"));
    }
    ...
    private void executeAction(String action, String uuidStr, String displayName, double amount, Ref<EntityStore> ref, Store<EntityStore> store) {
        ...
        switch (action) {
            case "Give" -> {
                ...
                playerRef.sendMessage(Message.raw(Messages.get("admin.give", "amount", Main.CONFIG.get().format(amount), "player", resolvedName)).color(Color.GREEN));
            ...
            case "Take" -> {
                ...
                if (success) {
                    playerRef.sendMessage(Message.raw(Messages.get("admin.take", "amount", Main.CONFIG.get().format(amount), "player", resolvedName)).color(Color.ORANGE));
                    lastFeedback = "-" + Main.CONFIG.get().formatShort(amount);
                } else {
                    playerRef.sendMessage(Message.raw(Messages.get("admin.insufficient_funds")).color(Color.RED));
                }
            }
            case "Set" -> {
                economyManager.setBalance(targetUuid, amount, "Admin set via GUI");
                playerRef.sendMessage(Message.raw(Messages.get("admin.set", "player", resolvedName, "amount", Main.CONFIG.get().format(amount))).color(Color.YELLOW));
                lastFeedback = "=" + Main.CONFIG.get().formatShort(amount);
            }
        }
    }
    ...
    private void executeReset(String uuidStr, String displayName, Ref<EntityStore> ref, Store<EntityStore> store) {
        ...
        playerRef.sendMessage(Message.raw(Messages.get("admin.reset", "player", resolvedName)).color(Color.GRAY));
        lastFeedback = Messages.get("gui.players.reset");
    }
